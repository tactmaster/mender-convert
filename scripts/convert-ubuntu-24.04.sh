#!/usr/bin/env bash
# Convert a raw Ubuntu 24.04 disk image to a Mender image using docker-mender-convert.
#
# Usage: ./scripts/convert-ubuntu-24.04.sh
#
# Requires:  input/Ubuntu-24.04-x86-64.img   (produced by install-ubuntu-24.04.sh)
# Produces:  deploy/Ubuntu-24.04-x86-64-qemux86-64-mender.img

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
INPUT_IMG="${REPO_ROOT}/input/Ubuntu-24.04-x86-64.img"
...
echo "Starting mender-convert for Ubuntu 24.04 LTS..."
echo "Input:  ${INPUT_IMG}"
echo ""

MENDER_ARTIFACT_NAME=release-1 ./docker-mender-convert \
    --disk-image input/Ubuntu-24.04-x86-64.img \
    --config configs/ubuntu-qemux86-64_config \
    ${OVERLAY_ARG}

echo ""
echo "Conversion complete. Output:"
ls -lh deploy/Ubuntu-24.04-x86-64-qemux86-64-mender.img 2>/dev/null \
    || ls -lh deploy/Ubuntu-24.04* 2>/dev/null \
