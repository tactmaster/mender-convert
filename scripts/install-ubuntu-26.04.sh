#!/usr/bin/env bash
# Install Ubuntu 26.04 to a raw disk image using QEMU/KVM + swtpm (TPM 2.0).
# The cloud-init autoinstall mechanism drives an unattended installation.
#
# Usage: ./scripts/install-ubuntu-26.04.sh
#
# Produces:  input/Ubuntu-26.04-x86-64.img
# Preserves: input/tpm-state-26.04/   (TPM state reused by boot script)
#            input/ovmf-nvram-26.04.fd (UEFI NVRAM with boot entries)

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ISO="${REPO_ROOT}/assets/ubuntu-26.04-live-server-amd64.iso"
OUTPUT_IMG="${REPO_ROOT}/input/Ubuntu-26.04-x86-64.img"
SEED_ISO="${REPO_ROOT}/work/ubuntu-26.04-seed.iso"
TPM_STATE_DIR="${REPO_ROOT}/input/tpm-state-26.04"
NVRAM="${REPO_ROOT}/input/ovmf-nvram-26.04.fd"
TPM_SOCKET="/tmp/tpm-26.04.sock"
IMG_SIZE_GB=12
...
# ---------------------------------------------------------------------------
# swtpm — auto-initialises state on first run, no swtpm_setup needed
# ---------------------------------------------------------------------------
rm -rf "$TPM_STATE_DIR"
...
# ---------------------------------------------------------------------------
echo ""
echo "Booting Ubuntu installer — this will take 10-20 minutes."
