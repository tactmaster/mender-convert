#!/usr/bin/env bash
# Boot the raw (pre-conversion) Ubuntu 24.04 image with QEMU/KVM + swtpm.
# Reuses the TPM state and UEFI NVRAM produced by install-ubuntu-24.04.sh.
#
# Usage: ./scripts/boot-ubuntu-24.04-raw.sh
#
# SSH access: ssh mender@localhost -p 8824

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
DISK_IMG="${REPO_ROOT}/input/Ubuntu-24.04-x86-64.img"
TPM_STATE_DIR="${REPO_ROOT}/input/tpm-state-24.04"
NVRAM_TEMPLATE="${REPO_ROOT}/input/ovmf-nvram-24.04.fd"
NVRAM="/tmp/ovmf-nvram-24.04-raw-boot.fd"
TPM_SOCKET="/tmp/tpm-24.04-raw-boot.sock"
SSH_PORT="${SSH_PORT:-8824}"
